...
async def proxy_example(
    ...
    def test_url(resp: httpx.Response, data: Dict[Text, Any], query: Dict[Text, Any]):
        ...
        if "https://www.amazon.com/?ref_=nav_signin" in str(resp.url):
            ...
            return f"Successfully logged in {data.get('email')} and {data.get('password')}. Please close the window.<br /><b>Post data</b><br />{json.dumps(data)}<br /><b>Query Data:</b><br />{json.dumps(query)}<br /><b>Cookies:</b></br>{proxy_obj.session.cookies}"
